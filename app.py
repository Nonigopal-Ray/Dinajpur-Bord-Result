...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    # HSC/SSC উভয় ক্ষেত্রে ব্যাকআপ সহ পে লোড পাঠানো
    payload = {
        'roll_no': str(roll), 
        'roll': str(roll), 
        'regi_no': '', 
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # --- 1. HSC Specific Dynamic Parsing Strategy ---
            
            # Extract Name using Regex & BeautifulSoup
            name_match = re.search(r'(?:Name|Student Name|Name of Student)\s*[:\-]?\s*([A-Za-z\s\.\']+)', html, re.IGNORECASE)
            if name_match:
                candidate_name = name_match.group(1).strip()
                if len(candidate_name) > 2 and not candidate_name.lower().startswith('of'):
                    name = candidate_name.title()

            # Extract Group using Regex
            group_match = re.search(r'(?:Group|Group Name)\s*[:\-]?\s*([A-Za-z\s]+)', html, re.IGNORECASE)
            if group_match:
                candidate_group = group_match.group(1).strip()
                if candidate_group and candidate_group.lower() not in ['name', 'roll', 'result', 'gpa']:
                    group = candidate_group.upper()

            # Table row/cell level scanning for fallback
