...
class ColorPDF(FPDF):
    ...
    def draw_colored_grades(self, x, y, width, height, text):
        ...
        for item in items:
            ...
            if current_x + item_width > x + width - 1:
                current_x = x + 1
                current_y += 4
            ...
            self.text(current_x, current_y + 2, item_str)
...
def generate_pdf(dataframe, institute_info, year):
    ...
    for _, row in dataframe.iterrows():
        ...
        items_count = len(sg.split(','))
        extra_lines = max(1, (items_count // 9) + 1)
        row_h = max(line_height, extra_lines * 4.5 + 2)
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        # Dynamically fetch functional backend processing links for the selected year (2024, 2025, or 2026)
        ...
        # STEP 1: Always Attempt EIIN Processing First for All Selected Years (2024, 2025, 2026)
        try:
            ...
            # Extract student rolls associated with the school
            ...
            # Check if valid school response was returned
        ...
        # STEP 2: Fallback to Roll Search if EIIN Processing Returned No Valid School Records
        ...
        # STEP 3: Process Individual Student Details via student action URL
        if extracted_pairs:
...
if 'df_results' in st.session_state and not st.session_state['df_results'].empty:
    ...
    <style>
        ...
        .custom-table { width: 100%; border-collapse: collapse; margin-top: 5px; font-size: 13px; text-align: center; }
        .custom-table th { background-color: #E0E0E0; color: #000; padding: 8px; border: 1px solid #BDBDBD; font-weight: bold; white-space: nowrap; }
        .custom-table td { padding: 8px; border: 1px solid #E0E0E0; white-space: nowrap; }
