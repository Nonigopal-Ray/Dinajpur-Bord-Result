...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    payload = {'roll_no': str(roll), 'roll': str(roll), 'regi_no': '', 'submit': 'Submit'}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Robust Parsing Strategy using Table Rows (TR/TD)
            ...
            for row in rows:
                ...
                if re.search(r'Name', row_text, re.IGNORECASE) and name == "Not Found":
                    if len(cols) >= 2:
                        val = cols[-1].get_text(strip=True)
                        val = re.sub(r'^(Name|Student Name|Name of Student)\s*[:\-]\s*', '', val, flags=re.IGNORECASE)
                        if len(val) > 1 and not val.isdigit():
                            name = val
                
                # Extract Group
                if re.search(r'Group', row_text, re.IGNORECASE) and group == "Not Found":
                    if len(cols) >= 2:
                        val = cols[-1].get_text(strip=True)
                        val = re.sub(r'^(Group|Group Name)\s*[:\-]\s*', '', val, flags=re.IGNORECASE)
                        if val:
                            group = val

                # Extract Result / GPA
                if re.search(r'(Result|GPA)', row_text, re.IGNORECASE) and result_val in ["-", "Not Found"]:
                    if len(cols) >= 2:
                        val = cols[-1].get_text(strip=True)
                        if val:
                            result_val = val
                ...
                # Extract Total Mark
                if re.search(r'Total\s*Mark', row_text, re.IGNORECASE):
                    if len(cols) >= 2:
                        val = cols[-1].get_text(strip=True)
                        if val:
                            total_mark_val = val

            # Fallback for Name, Group, Result if TR iteration missed them
            all_tds = soup.find_all('td')
            if name == "Not Found" or group == "Not Found":
            ...
            # Format Result (GPA vs Fail Count)
            if compulsory_failed_count > 0:
                result_val = f"F{compulsory_failed_count}"
            elif "PASSED" in result_val.upper() or "GPA=" in result_val.upper() or "GPA" in result_val.upper():
