...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # --- টেবিলের সেল ধরে তথ্য এক্সট্র্যাক্ট করার উন্নত লজিক ---
            tables = soup.find_all('table')
            
            if tables:
                # ১ম টেবিলে সাধারণত শিক্ষার্থীর ব্যক্তিগত তথ্য ও সার্বিক রেজাল্ট থাকে
                main_table = tables[0]
                ...
                for row in rows:
                    ...
                    for i, cell in enumerate(cells):
                        ...
                        # ডান পাশের সেল বা পরের সেলে মান খোঁজা
                        next_cell_val = cells[i + 1].get_text(strip=True) if (i + 1 < len(cells)) else ""
                        
                        # Student Name এক্সট্র্যাক্ট
                        if any(k in cell_lower for k in ['name of student', 'student name', 'name:']):
                            if next_cell_val and name == "Not Found":
                                name = next_cell_val
                        
                        # Group Name এক্সট্র্যাক্ট
                        elif 'group' in cell_lower and 'group name' not in cell_lower:
                            if next_cell_val and group == "Not Found":
                                group = next_cell_val
                        
                        # Result / GPA এক্সট্র্যাক্ট
                        elif any(k in cell_lower for k in ['result', 'gpa', 'gpa=']):
                            if next_cell_val:
                                result_val = next_cell_val
                        
                        # Total Mark এক্সট্র্যাক্ট
                        elif 'total mark' in cell_lower:
                            if next_cell_val:
                                total_mark_val = next_cell_val

            # --- বিষয়ভিত্তিক গ্রেড এক্সট্র্যাক্ট (সাবজেক্ট টেবিল থেকে) ---
            subject_grades = []
            ...
            if len(tables) > 1:
                ...
                for r in sub_rows:
                    ...
                    if len(tds) >= 2:
                        # Subject Name এবং তার ডানের সেলে Subject Grade চেক করা
                        sub_name = ""
                        grade = ""
                        ...
                        if clean_grade in ['A+', 'A', 'A-', 'B', 'C', 'D', 'F']:
                            subject_grades.append(f"{sub_name}:{clean_grade}")
                            if clean_grade == 'F':
            ...
            # GPA ক্লিন করা
            m_gpa = re.search(r'(?:GPA\s*=\s*|GPA\s*)?([\d\.]+)', result_val, re.IGNORECASE)
            if m_gpa and result_val != "F":
            ...
            return {
                "Group": group.upper(),
                "Roll": str(roll),
                "Name": name.title() if name != "Not Found" else name,
                "GPA": result_val,
