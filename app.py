...
def sanitize_filename(name):
    if not name or name in ["Unknown Institute", "Individual Roll Result Sheet"]:
        return "School_Result"
    clean_name = re.sub(r"[^\w\s-]", "", name)
    clean_name = re.sub(r"[-\s]+", "_", clean_name).strip("_")
    return clean_name if clean_name else "School_Result"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, td in enumerate(all_tds):
                ...
                if (
                    ("institute" in txt or "school" in txt)
                    and i + 1 < len(all_tds)
                    and extracted_inst == "-"
                ):
                    possible_inst = all_tds[i + 1].text.strip()
                    if len(possible_inst) > 3 and not possible_inst.isdigit():
                        extracted_inst = possible_inst
            ...
            return {
                ...
                "Extracted_Institute": extracted_inst,
            }
        else:
            return {
                "Group": "-",
...
def generate_pdf(dataframe, institute_info, year):
    ...
    # ১. স্কুলে নাম পিডিএফে প্রাধান্য দিয়ে দেখানো
    ...
    pdf.cell(0, 7, inst_name[:80], ln=True, align="C")
    ...
    pdf.cell(
        0,
        5,
        f"EIIN: {eiin} | Thana/Upazilla: {thana} | District: {dist}",
        ln=True,
        align="C",
    )
    pdf.ln(2)
        ...
        if extracted_pairs:
            ...
            with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
                ...
                for i, future in enumerate(futures):
                    ...
                    # যদি বোর্ডের সামারিতে স্কুলের নাম মিস থাকে, তবে স্টুডেন্ট রেজাল্ট থেকে তা রিকভার করা
                    if (
                        institute_info.get("Institute") == "Unknown Institute"
                        and res_item.get("Extracted_Institute") != "-"
                    ):
                        institute_info["Institute"] = res_item["Extracted_Institute"]
...
if (
    ...
    # ২. ওয়েবসাইটে স্কুলের নাম স্পষ্টভাবে কার্ড আকারে প্রদর্শন
    school_name_display = info.get("Institute", "Institute Result Sheet")
    st.markdown(
        f"""
        <div class="school-card">
            <h2 style="margin:0; color:#1A237E; font-size:22px;">🏫 {school_name_display}</h2>
            <p style="margin:5px 0 0 0; color:#555; font-size:14px;">
                <b>EIIN:</b> {eiin_code} &nbsp;|&nbsp; 
                <b>Thana/Upazilla:</b> {info.get('Thana/Upazilla', '-')} &nbsp;|&nbsp; 
                <b>District:</b> {info.get('District', '-')}
            </p>
        </div>
    """,
        unsafe_allow_html=True,
    ...
    # ৩. ডাইনামিক ফাইল নেম তৈরি (স্কুলের নাম সহ)
    clean_school_name = sanitize_filename(info.get("Institute"))
    if is_eiin_search:
        pdf_filename = f"{clean_school_name}_SSC_{year}_EIIN_{eiin_code}.pdf"
    else:
    ...
    with col1:
        try:
            ...
            st.download_button(
                label=f"📥 Download PDF ({clean_school_name})",
