...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # --- 1. Top Table Parsing (Name, Group, GPA, Total Mark) ---
            ...
            if tables:
                ...
                for row in rows:
                    ...
                    for idx, text in enumerate(cols):
                        ...
                        if 'name of student' in text_lower or 'student name' in text_lower:
                            if idx + 1 < len(cols) and cols[idx+1]:
                                name = cols[idx+1]
                        
                        # Extract Group
                        elif 'group' in text_lower and 'group name' not in text_lower:
                            if idx + 1 < len(cols) and cols[idx+1]:
                                group = cols[idx+1]
                        
                        # Extract GPA / Result
                        elif 'result' in text_lower or 'gpa' in text_lower:
                            if idx + 1 < len(cols) and cols[idx+1]:
                                raw_res = cols[idx+1]
                                match = re.search(r'GPA\s*=\s*([\d\.]+)', raw_res, re.IGNORECASE)
                                if match:
                                    result_val = match.group(1)
                                elif 'PASSED' in raw_res.upper() or 'FAIL' in raw_res.upper():
                                    result_val = raw_res
                                else:
                                    result_val = raw_res
                        
                        # Extract Total Mark
                        elif 'total mark' in text_lower or 'total_mark' in text_lower:
                            if idx + 1 < len(cols) and cols[idx+1]:
                                total_mark_val = cols[idx+1]

            # --- Fallback Parsing using Regex (if table structure misses anything) ---
            if name == "Not Found":
                m = re.search(r'Name of Student\s*[:\-]?\s*([A-Za-z\s\.\']+)', html, re.IGNORECASE)
            ...
            if total_mark_val == "-":
                m = re.search(r'TOTAL MARK\s*[:\-]?\s*(\d+)', html, re.IGNORECASE)
                if m: total_mark_val = m.group(1).strip()

            # --- 2. Subject Table Parsing (CODE, SUBJECT, MARK/GRADE) ---
            subject_grades = []
            ...
            if len(tables) > 1:
                ...
                for r in sub_rows[1:]:  # Skip Header Row
                    tds = [c.get_text(strip=True) for c in r.find_all(['td', 'th'])]
                    if len(tds) >= 3:
                        sub_name = tds[1]
                        grade = tds[2]
                        
                        # Valid Letter Grade Check
            ...
            # Formulating Final Result Status
            if compulsory_failed_count > 0:
                result_val = f"F{compulsory_failed_count}"
            elif "PASSED" in result_val.upper():
                m = re.search(r'\d+\.\d+|\d+', result_val)
                if m: result_val = m.group(0)
