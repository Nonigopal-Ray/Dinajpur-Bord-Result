...
def get_board_urls(exam_type, year):
    exam_prefix = "ssc" if exam_type == "SSC" else "hsc"
    
    institute_url = f"https://result.dinajpurboard.gov.bd/{exam_prefix}_result_{year}/search/search.php"
    
    if exam_type == "HSC":
        student_action_url = f"https://result.dinajpurboard.gov.bd/hsc_result_{year}/search/each.php"
    else:
...
# Custom PDF Class with fixed Cell Overflow Bug
class ColorPDF(FPDF):
    ...
    def draw_colored_grades(self, x, y, width, height, text):
        ...
        for item in items:
            ...
            self.text(current_x, current_y + 2.2, item_str)
...
def generate_pdf(dataframe, institute_info, exam_type, year):
    ...
    for _, row in dataframe.iterrows():
        ...
        # Calculate strict row height to keep everything strictly inside cells
        items = [i.strip() for i in sg.split(',') if i.strip()]
        lines_needed = max(1, (len(items) // 8) + 1)
        row_h = max(8, lines_needed * 3.8 + 2.5)
        ...
        # Cell 6: Subject Grade (Draw Outer Border Box + Render Text inside)
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        # Get dynamic backend links based on Exam Type & Year
        ...
        # STEP 1: Attempt EIIN Processing FIRST (Both for SSC and HSC)
        try:
            ...
            if pairs and ("Institute" in raw_text or "EIIN" in raw_text):
                is_eiin_search = True
        ...
        # STEP 2: Fallback to Individual Roll Search if EIIN failed
        ...
        # STEP 3: Process Individual Student Details
        if extracted_pairs:
