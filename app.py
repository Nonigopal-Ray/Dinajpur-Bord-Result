...
# Fixed ColorPDF Class with improved Grade Box wrapping & spacing
class ColorPDF(FPDF):
...
def generate_pdf(dataframe, institute_info, exam_type, year):
    ...
    for _, row in dataframe.iterrows():
        ...
        # Calculate dynamic height so no subject grades are cut off
        items_count = len([i for i in sg.split(',') if i.strip()])
        lines_needed = max(1, (items_count // 8) + 1)
        row_h = max(8, lines_needed * 4.2 + 2.5)
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        # STEP 1: Attempt EIIN Processing
        try:
        ...
        # STEP 2: Fallback to Roll Search if EIIN failed
