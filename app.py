...
# Improved PDF Class: Guarantees no subject grade is missed & no overflow
class ColorPDF(FPDF):
...
def generate_pdf(dataframe, institute_info, exam_type, year):
    ...
    for _, row in dataframe.iterrows():
        ...
        # Exact calculation for line wrapping inside PDF cell
        items = [i.strip() for i in sg.split(',') if i.strip()]
        lines_needed = max(1, (len(sg) // 105) + 1)
        ...
        # Subject Grade Cell with absolute boundary logic
...
if st.button("Search Result 🚀"):
    ...
    if not raw_input:
        ...
        # STEP 1: STRICT EIIN SEARCH (FOR BOTH SSC & HSC)
        try:
            # Multi-parameter payload to ensure dynamic server acceptance
            payload = {
            ...
            # Check if response contains valid Institute Data
