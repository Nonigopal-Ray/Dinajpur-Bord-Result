...
# ---------------------------------------------------------
MAX_THREADS = 8  # সার্ভার ডাউন/স্লো হওয়া এড়াতে থ্রেড কিছুটা কমানো হয়েছে

OPTIONAL_SUBJECT_KEYWORDS = [
    "agriculture", "agricultural", "higher math", "higher mathematics", 
    "home science", "computer", "biology", "physical education", "stat", "statistics"
]

def get_board_urls(exam_type, year):
    if exam_type.upper() == "HSC":
...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    # SSC ও HSC উভয়ের জন্য সম্ভাব্য সমস্ত ফর্ম কী-ওয়ার্ড যুক্ত প্যানিক পেলোড
    payload = {
        'roll_no': str(roll), 
        'roll': str(roll), 
        'r': str(roll),
    ...
    try:
        # টাইমাউট ১৫ সেকেন্ড করা হয়েছে যাতে স্লো রেসপন্সে কানেকশন ড্রপ না করে
        response = requests.post(action_url, data=payload, headers=session_headers, timeout=15)
        if response.status_code == 200:
            html = response.text
            soup = BeautifulSoup(html, 'html.parser')
            
            # কোনো ডাটা না থাকলে
            if "no result found" in html.lower() or "not found" in html.lower() and len(html) < 500:
                return {
            ...
            # --- SSC & HSC উভয়ের জন্য কমন কি-ওয়ার্ড বেসড পার্সার ---
            all_tds = soup.find_all(['td', 'th'])
            for i, td in enumerate(all_tds):
                txt = td.get_text(strip=True)
                txt_lower = txt.lower()
                
                # Student Name এক্সট্র্যাক্ট
                if any(k in txt_lower for k in ['name of student', 'student name', 'name:']):
                    if i + 1 < len(all_tds):
                        val = all_tds[i+1].get_text(strip=True)
                        if len(val) > 1 and name == "Not Found":
                            name = val
                
                # Group Name এক্সট্র্যাক্ট
                elif 'group' in txt_lower and 'group name' not in txt_lower:
                    if i + 1 < len(all_tds):
                        val = all_tds[i+1].get_text(strip=True)
                        if len(val) > 1 and group == "Not Found":
                            group = val
                
                # Result / GPA এক্সট্র্যাক্ট
                elif any(k in txt_lower for k in ['result', 'gpa=']):
                    if i + 1 < len(all_tds):
                        val = all_tds[i+1].get_text(strip=True)
                        if 'gpa' in val.lower() or re.search(r'\d+\.\d+', val):
                            result_val = val
                
                # Total Mark এক্সট্র্যাক্ট
                elif 'total mark' in txt_lower:
                    if i + 1 < len(all_tds):
                        val = all_tds[i+1].get_text(strip=True)
                        if val.isdigit():
                            total_mark_val = val

            # --- ব্যাকআপ রেজেক্স স্ট্র্যাটেজি (যদি এইচটিএমএল টেবিল মিস করে) ---
            if name == "Not Found":
                m = re.search(r'(?:Name of Student|Student Name|Name)\s*[:\-]?\s*([A-Za-z\s\.\']+)', html, re.IGNORECASE)
                if m: name = m.group(1).strip()
                
            if group == "Not Found":
                m = re.search(r'(?:Group)\s*[:\-]?\s*([A-Za-z]+)', html, re.IGNORECASE)
                if m: group = m.group(1).strip()

            if total_mark_val == "-":
                m = re.search(r'(?:TOTAL MARK|Total Mark)\s*[:\-]?\s*(\d+)', html, re.IGNORECASE)
                if m: total_mark_val = m.group(1).strip()

            # GPA ক্লিন করা
            m_gpa = re.search(r'GPA\s*=\s*([\d\.]+)', result_val, re.IGNORECASE)
            if m_gpa:
                result_val = m_gpa.group(1)

            # --- সাবজেক্ট ওয়াইজ গ্রেড এক্সট্র্যাক্ট ---
            subject_grades = []
            compulsory_failed_count = 0
            
            # টেবিলের দ্বিতীয় অংশ (Subject Table) থেকে পার্সিং
            tables = soup.find_all('table')
            if len(tables) > 1:
                sub_table = tables[1]
                for r in sub_table.find_all('tr'):
                    tds = [c.get_text(strip=True) for c in r.find_all(['td', 'th'])]
                    if len(tds) >= 3:
                        sub_name = tds[1]
                        grade = tds[2]
                        if grade in ['A+', 'A', 'A-', 'B', 'C', 'D', 'F']:
                            subject_grades.append(f"{sub_name}:{grade}")
                            if grade == 'F':
                                is_optional = any(opt in sub_name.lower() for opt in OPTIONAL_SUBJECT_KEYWORDS)
                                if not is_optional:
        ...
        return {"Group": "-", "Roll": str(roll), "Name": "Timeout/Connection Error", "GPA": initial_status, "Total Mark": "-", "Subject Grade": "-"}
