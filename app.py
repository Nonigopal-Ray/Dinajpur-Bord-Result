...
def get_board_urls(year):
    if str(year) == "2026":
        institute_url = "https://results.dinajpurboard.gov.bd/search/institute"[cite: 1]
        student_action_url = (
...
def solve_captcha(question_text):
    """ক্যাপচা থেকে গাণিতিক সমীকরণ সমাধান করা (যেমন: 2 + 2 = ?)"""
...
def fetch_student_detail(
    ...
    try:
        if str(year) == "2026":
            # ১. CSRF Token এবং Captcha সংগ্রহের জন্য GET রিকোয়েস্ট
    ...
    if not raw_input:
        ...
        try:
            if selected_year == "2026":
                # ২০২৬ সালের জন্য GET দিয়ে CSRF টোকেন ফেচ করে POST রিকোয়েস্ট
                get_res = session.get(
                    institute_url, headers=headers, timeout=12
                )
                soup_init = BeautifulSoup(get_res.text, "html.parser")
                token_input = soup_init.find("input", {"name": "_token"})
                csrf_token = token_input["value"] if token_input else ""[cite: 1]

                payload = {
                    "_token": csrf_token,[cite: 1]
                    "eiin_no": first_candidate,[cite: 1]
                    "submit": "1",[cite: 1]
                }
                res = session.post(
            ...
            if pairs and (
                "Institute" in raw_text or "board-panel-title" in raw_text
            ):
                is_eiin_search = True
