...
def fetch_student_detail(roll, action_url, session_headers, initial_status="-"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # --- Robust HTML Table Extraction for HSC & SSC ---
            ...
            for row in rows:
                ...
                if len(cols) >= 2:
                    ...
                    if 'name' in label and 'father' not in label and 'mother' not in label and 'institute' not in label:
                        if len(val) > 1 and name == "Not Found":
                            name = val.title()
                    elif 'group' in label:
                        if len(val) > 1 and group == "Not Found":
                            group = val.upper()
                    elif 'gpa' in label or 'result' in label:
                        if len(val) > 0 and result_val == "-":
                            result_val = val
                    elif 'total' in label and 'mark' in label:
                        if len(val) > 0:
                            total_mark_val = val

            # Fallback Regex Search for Name & Group if Table Parsing Misses
            if name == "Not Found":
                name_match = re.search(r'(?:Name|Student Name)\s*[:\-]\s*([A-Za-z\s\.\']+)', html, re.IGNORECASE)
                if name_match:
                    name = name_match.group(1).strip().title()

            if group == "Not Found":
                group_match = re.search(r'(?:Group)\s*[:\-]\s*([A-Za-z]+)', html, re.IGNORECASE)
                if group_match:
                    group = group_match.group(1).strip().upper()
